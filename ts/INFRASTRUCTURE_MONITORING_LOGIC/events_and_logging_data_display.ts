...
let globalEventsCadenceStatsGraphInstance : any = null;

export async function openEventsSection(event : Event) {
    genericScript.switchSection('tasks.html', event);
    const globalEventsInfo : typeDefinitions.eventsAndLoggingDashboard = await userDataRetrieval.getUserGlobalEventsData() as typeDefinitions.eventsAndLoggingDashboard;
...
function displayGlobalEventsActivity(recentActivityData : Array<typeDefinitions.GlobalEvent>) {
...
function displayGlobalEventsDistributionGraph(eventTypesDistribution : typeDefinitions.GlobalEventTypesDistribution) {
    const eventDistributionChartCanvas2DContext = (document.getElementById('eventDistributionViewCanvas') as HTMLCanvasElement)!.getContext('2d');
...
function displayGlobalEventsCadenceStatsGraph(eventCadenceStats : Array<typeDefinitions.GlobalEventCadenceRegistry>, timeScaleUnit = "day") {
    ...
    if(eventCadenceStats.length > 0) {
        ...
        const eventCadenceCanvas = (document.getElementById('eventCadenceViewCanvas') as HTMLCanvasElement)!.getContext('2d');
...
async function changeGlobalEventsCadenceGraphInterval(interval : Number) {
    ...
    const globalEventsCadenceStats : Array<typeDefinitions.GlobalEventCadenceRegistry> = await userDataRetrieval.getUserGlobalEventsCadenceStatsData(interval) as Array<typeDefinitions.GlobalEventCadenceRegistry>;
    displayGlobalEventsCadenceStatsGraph(globalEventsCadenceStats, timeScaleUnit);
}

function displayNotificationLogsData(notificationLogsData : Array<typeDefinitions.NotificationInfo>) {
...
function displayScheduledBatchOperationsData(scheduledBatchOperationsData : Array<typeDefinitions.PPoolsBatchOperationHistory>) {
