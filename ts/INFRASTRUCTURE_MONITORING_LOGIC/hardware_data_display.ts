...
export async function openHardwareSection(event : Event | null) {
...
async function displayRecentActivity() {
    const recentActivityData : Array<typeDefinitions.GlobalEvent> = await dataRetrievalScript.getRecentActivity() as Array<typeDefinitions.GlobalEvent>;
...
function createNewPGrid() {
    const pgridCreationWizzardSubmitAction = async (e : Event, selfInstance : editorComponents.sideConfigurationEditWizzard) => {
        const pgridName = (document.getElementById("pgridNameField") as HTMLInputElement)!.value;
        const pgridReadmeTXT = (document.getElementById("pgridReadmeTextField") as HTMLTextAreaElement)!.value;
